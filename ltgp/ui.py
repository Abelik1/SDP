...
class LTGPMainWindow(QWidget):
        ...
        try:
            if os.path.exists(FAV_FILE):
                with open(FAV_FILE, 'r', encoding='utf-8') as f:
                    self._favorites = set(json.load(f).get('favorites', []))
        except Exception:
            self._favorites = set()
    ...
    def _build_ui(self):
        ...
        self.search.setPlaceholderText('Filter by title / id / tag')
        ...
        self.custom_json.setMinimumHeight(70)
        ...
        self.classical = QCheckBox('Energy-diagonal / commuting restriction (where applicable)')
        self.classical.setChecked(False)
        form.addRow('', self.classical)
        self.reset_system = QCheckBox('Reset system on Run')
        ...
        self.local_mode = QComboBox(); self.local_mode.addItems(['verified', 'multistart', 'single']); self.local_mode.setCurrentText('verified')
        form.addRow('Solver hint:', self.solver)
        form.addRow('Local edge mode:', self.local_mode)

        self.extra_vars = QLineEdit()
        self.extra_vars.setPlaceholderText('Extra vars: k=v, k2=v2 ...')
        form.addRow('Extra vars:', self.extra_vars)

        self.vars_preview = QLineEdit()
        ...
        self.warning.setMaximumHeight(72)
        self.warning.setPlainText('Report-facing rule: use verified local edges. The single local solver is heuristic only and can produce false positives.')
        right_layout.addWidget(QLabel('Method warning:'))
        ...
        self.results = QTextEdit(); self.results.setReadOnly(True); self.results.setMinimumHeight(280)
    ...
    def _rebuild_tree(self):
        ...
        leaf = fav_root.child(0) if fav_root.childCount() > 0 else None
        if leaf is None:
            for i in range(self.tree.topLevelItemCount()):
                top = self.tree.topLevelItem(i)
                if top.childCount() > 0 and top is not fav_root:
                    leaf = top.child(0)
    ...
    def _apply_filter(self):
        ...
        if not q:
            ...
            return
        catalog = get_catalog()
        tag_map = {s.eq_id: ' '.join([s.eq_id, s.title, ' '.join(s.tags)]).lower() for _, specs in catalog.items() for s in specs}
        for i in range(self.tree.topLevelItemCount()):
            top = self.tree.topLevelItem(i)
            any_visible = False
            for k in range(top.childCount()):
    ...
    def _on_tree_selection(self):
        ...
        self.details.setPlainText(f'{spec.title}\n\n{spec.description}' if spec else f'Unknown experiment id: {eq_id}')
    ...
    def _on_mode_changed(self):
        self.custom_json.setEnabled(self.mode.currentIndex() == 1)
    ...
    def _on_run(self):
        ...
        try:
            ...
            if isinstance(res, dict):
                ...
                if arts:
                    text += '\n\nArtifacts:\n' + '\n'.join(f'- {k}: {v}' for k, v in arts.items() if v)
