...
class LTSDPWindow(QMainWindow):
    ...
    def _params_box(self) -> QGroupBox:
        ...
        # Report export
        self.export_pdf = QCheckBox("export PDF report")
        self.export_pdf.setChecked(False)

        self.report_stem = QLineEdit()
        self.report_stem.setPlaceholderText("e.g. run_001")

        self.report_outdir = QLineEdit()
        self.report_outdir.setPlaceholderText("e.g. reports")

        grid.addWidget(self.export_pdf, r, 0, 1, 2)
        grid.addWidget(QLabel("report stem"), r, 2)
        grid.addWidget(self.report_stem, r, 3)
        r += 1

        grid.addWidget(QLabel("report outdir"), r, 0)
        grid.addWidget(self.report_outdir, r, 1, 1, 3)
        r += 1
    ...
    def _build_variables_string(self) -> str:
        ...
        pairs.append(_kv("export_pdf", self.export_pdf.isChecked()))
        # For paths, keep them as plain strings. Backend can ignore if empty.
        if self.report_stem.text().strip():
            pairs.append(_kv("report_stem", self.report_stem.text().strip()))
        if self.report_outdir.text().strip():
            pairs.append(_kv("report_outdir", self.report_outdir.text().strip()))
    ...
    def _build_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "report_stem": self.report_stem.text().strip(),
            "report_outdir": self.report_outdir.text().strip(),
        }
